...
class OrdersUpdater(orders_channel.OrdersProducer):
    ...
    async def _order_fetch_and_push(self, order, should_notify=False):
        ...
        if raw_order is None:
            if order.exchange_manager.exchange.CONNECTOR_CONFIG \
                .ORDER_NOT_FOUND_SETS_THE_ORDER_TO_CANCELED:
                self.logger.debug(f"Received update for {order} on {exchange_name}: "
                                "The order was not found, the state will be set to canceled")
                order.consider_as_canceled()
                await self.channel.exchange_manager.exchange_personal_data.handle_order_instance_update(
                    order, should_notify=should_notify
                )
            else:
                raise errors.OrderNotFoundError
        else:
